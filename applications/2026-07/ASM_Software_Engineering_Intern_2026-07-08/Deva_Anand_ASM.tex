...
\begin{itemize}
  ...
  \item Architected a full-stack internal tooling platform (React, Node.js, MongoDB) used daily by the AI team, cutting manual work by \textbf{100+ hours/month} and lifting throughput by \textbf{60\%}.
  \item Hardened backend security and reliability: \textbf{AWS KMS} encryption for sensitive data, auth and rate-limit hardening, and \textbf{20+ VAPT} remediations, with integration tests for validation and production log monitoring for incident visibility.
  \item Served as primary technical POC, wrote knowledge-transfer documentation onboarding a second developer in \textbf{$\sim$6 weeks}, and piloted a COO-sponsored GitHub Copilot experiment for AI-assisted development.
\end{itemize}
...
\begin{itemize}
  \item Built and deployed \textbf{Underdogs Fitness} (\href{https://www.underdogsfitness.in/}{Website}), a production MERN platform with \textbf{git-webhook build/deploy automation} (GitHub webhook to a droplet on push), cron automation, and Stripe payments; live \textbf{3+ years}.
\end{itemize}
...
\begin{itemize}
  \item Built a Python/FastAPI backend service with automated provider routing/fallback and pipeline instrumentation for \textbf{log visibility}; validated and hardened with \textbf{150+ automated tests}.
\end{itemize}
...
\begin{itemize}
  \item Built an automation tool that proposes and validates changes, scores them against multiple reference systems, and commits only improvements to \textbf{Git} with report cards and guardrails, keeping validation reproducible and auditable.
\end{itemize}
...
\begin{itemize}
  \item Optimized C++ matrix multiplication (systems programming) through cache-aware tiling, \textbf{AVX/SIMD} vectorization, and \textbf{OpenMP}, measuring and validating L1/L3/TLB performance with \textbf{perf}.
\end{itemize}
...
%----------SKILLS----------
\section{Skills}
\skillLine{Languages \& Databases}{Python, C++, TypeScript, JavaScript, SQL, Java, Bash, PostgreSQL, MongoDB, SQLite}
\skillLine{Frameworks / Tools}{FastAPI, Node.js, Express, React, Docker, Git, GitHub Actions, REST APIs}
\skillLine{Automation / Testing}{pytest, integration testing, CI/CD, Git automation, git-webhook deploy, perf profiling, debugging}
\skillLine{Backend / Observability}{REST APIs, authentication, rate limiting, observability (logging / monitoring), Linux, security}
