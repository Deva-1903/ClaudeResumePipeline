...
\begin{itemize}
  \item Took technical ownership of a multi-tenant NHS eTriage backend (Node.js, MongoDB) for \textbf{8+ UK clinical clients} from design through deployment and iteration, processing \textbf{10{,}000+ monthly triage submissions} via Mayden iaptus REST integrations; contributed to \textbf{\$340K+} in revenue.
  \item Built a full-stack user-facing annotation platform (React, Node.js, MongoDB) used daily by the AI team, cutting manual annotation by \textbf{100+ hours/month} and lifting labeling throughput by \textbf{60\%}.
...
\begin{itemize}
  \item Built and shipped \textbf{Underdogs Fitness} (\href{https://www.underdogsfitness.in/}{Website}) end-to-end, a customer-facing MERN gym-management product with Stripe + manual-cash hybrid payments, cron automation, and multi-branch support; live in production \textbf{3+ years}.
\end{itemize}
...
\begin{itemize}
  \item Built a cross-platform desktop product end-to-end with an Electron/React frontend and FastAPI backend, integrating camera, microphone, and an on-device pipeline; designed async APIs and queues to keep the UI fast and responsive under real-time video and audio streams.
\end{itemize}
...
\begin{itemize}
  \item Built an edge-native full-stack AI assistant in TypeScript that streams multi-step research digests to the client over WebSocket in real time, with durable server-side workflows and D1-backed state surviving process restarts and tool failures.
\end{itemize}
...
\begin{itemize}
  \item Built a Python/FastAPI backend service turning open-ended queries into structured entity tables with a reviewer-facing trust UI and \textbf{cell-level provenance}; hardened with \textbf{150+ tests} and provider fallback across LLM backends.
\end{itemize}
...
%----------SKILLS----------
\section{Skills}
\skillLine{Languages \& Databases}{Python, TypeScript, JavaScript, Go, Java, C++, SQL, Bash, PostgreSQL, MongoDB}
\skillLine{Frameworks / Tools}{React, Node.js, Express, FastAPI, Fastify, REST APIs, Git, GitHub Actions, Docker}
\skillLine{ML \& AI}{PyTorch, Hugging Face Transformers, LLM integration, RAG, prompt engineering, evaluation}
\skillLine{Testing / Practices}{pytest, integration testing, LambdaTest, Agile / Scrum, CI/CD, code review}
