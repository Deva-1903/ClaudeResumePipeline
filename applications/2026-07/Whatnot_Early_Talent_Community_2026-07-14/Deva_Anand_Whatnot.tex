...
\begin{itemize}
  \item Founding engineer at an early-stage healthtech startup, building full-stack product (Next.js + Firebase/GCP) for a provider-facing \textbf{SMART on FHIR} app that surfaces covered, guideline-backed medication alternatives inside the EHR prescribing flow to prevent prior authorization.
...
\begin{itemize}
  \item Sole engineer building the lab's CCR (Contextualized Construct Representations) text-analysis platform (Python, \textbf{FastAPI}, sentence-transformers, React) end to end, owning architecture and technical decisions while productionizing a published NLP method into a self-serve web tool for researchers.
...
\begin{itemize}
  \item Built a cross-platform desktop app with a fully offline, on-device pipeline (MediaPipe hand tracking, whisper.cpp ASR, local TTS) on a FastAPI + Electron/React stack, designing async APIs and queues to keep the UI responsive; shipped end to end under time pressure at the Qualcomm Edge AI Hackathon 2025.
\end{itemize}
...
\begin{itemize}
  \item Cut end-to-end Spark ETL runtime \textbf{$\sim$25\%} (17--19 min down to 14.3 min) on a \textbf{$\sim$1.4B-row} NYC TLC dataset via broadcast joins, AQE, skew-join handling, and shuffle-partition and column-pruning tuning, profiling stage-level timings in Spark UI.
\end{itemize}
...
%----------SKILLS----------
\section{Skills}
\skillLine{Languages}{Python, TypeScript, JavaScript, Java, C++, SQL, Bash}
\skillLine{Backend \& Web}{Node.js, Express, FastAPI, React, Next.js, REST APIs, WebSockets}
\skillLine{Databases}{MongoDB, PostgreSQL, Redis, SQLite}
\skillLine{Cloud \& DevOps}{AWS, GCP, Azure, Docker, Git, GitHub Actions, Kafka, Kubernetes}
\skillLine{ML \& AI}{PyTorch, sentence-transformers, Hugging Face, LLM agents, RAG, evaluation}

%----------OPEN SOURCE & COMMUNITY----------
\section{Open Source \& Community}
\inlineBlock{- Built \textbf{Underdogs Fitness} (\href{https://www.underdogsfitness.in/}{Website}), a production MERN platform with Stripe payments; live \textbf{3+ years}.}
