...
\begin{itemize}
  \item Integrated LLM-powered context-aware replies into a chat product and shipped a production LLM auto-commenting feature (\textbf{4-bit quantized Llama-7B via llama.cpp}) with prompt iteration, async queued generation, and content-safety filtering.
...
\begin{itemize}
  \item Built a multi-stage agentic service that turns open-ended queries into structured entity tables via retrieval planning, reranking, and deterministic extraction with LLM fallback and \textbf{cell-level provenance}; hardened with \textbf{150+ tests}.
\end{itemize}
...
\begin{itemize}
  \item Built the multi-view retrieval component of a KG-guided RAG pipeline (sub-question decomposition, RRF fusion, cross-encoder reranking, MMR), lifting supporting-fact recall \textbf{+2.68\%} over the KG\textsuperscript{2}RAG baseline on 4{,}905 questions.
\end{itemize}
...
\begin{itemize}
  \item Ran a mechanistic-interpretability study of Llama-3.1-8B-Instruct in PyTorch, extracting the residual-stream refusal direction via difference-in-means (refusal \textbf{0.92 $\to$ 0.32} at $k{=}3$) with causal activation patching and significance testing.
\end{itemize}
...
\skillLine{ML \& AI}{PyTorch, TensorFlow, scikit-learn, Hugging Face Transformers, JAX, NLP, deep learning, computer vision}
\skillLine{LLM \& Retrieval}{RAG, LLM APIs (OpenAI / Groq / Ollama), prompt engineering, RRF, cross-encoder reranking, MMR}
