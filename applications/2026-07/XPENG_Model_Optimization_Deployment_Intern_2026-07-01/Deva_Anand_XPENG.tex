...
\begin{itemize}
  ...
  \item Monitored production systems and debugged tenant-specific incidents; hardened auth and rate limiting, added \textbf{AWS KMS} encryption for PII/clinical data, and remediated \textbf{20+ VAPT findings}.
...
\begin{itemize}
  \item Shipped a production LLM feature running a \textbf{4-bit quantized Llama-7B via llama.cpp} on a single VM under constrained compute, with async queued generation, per-persona prompts, and content-safety filtering.
...
\projItem{Sonare -- Offline On-Device Sign / Speech App {\normalfont\small(\href{https://github.com/Deva-1903/Qualcomm-Sep25-Team-Sonare}{GitHub})}}{React, Electron, FastAPI, MediaPipe, whisper.cpp}
\begin{itemize}
  \item Built a cross-platform desktop app with a fully \textbf{on-device inference pipeline} (MediaPipe hand tracking, gesture classification, whisper.cpp ASR, local TTS) running offline with no network dependencies; designed async APIs and queues to keep the UI responsive under real-time video and audio streams.
\end{itemize}
...
\begin{itemize}
  \item Probed the internals of Llama-3.1-8B-Instruct, extracting the residual-stream refusal direction via difference-in-means (refusal \textbf{0.92 $\to$ 0.32} at $k{=}3$) with causal activation patching through PyTorch forward hooks, bootstrap 95\% CIs, and McNemar's test.
\end{itemize}
...
\begin{itemize}
  \item Cut end-to-end Spark ETL runtime \textbf{$\sim$25\%} (17--19 min to 14.3 min) on a \textbf{$\sim$1.4B-row} dataset via broadcast joins, AQE, and skew-join handling, using Spark UI stage timings to localize shuffle and skew bottlenecks and reduce the tail-latency ratio \textbf{2.14x to 1.74x}.
\end{itemize}
...
%----------SKILLS----------
\section{Skills}
\skillLine{Languages \& Databases}{Python, C++, TypeScript, JavaScript, Java, SQL, Bash, PostgreSQL, MongoDB, SQLite}
\skillLine{Frameworks / Tools}{FastAPI, Node.js, Express, Docker, Git, GitHub Actions, pytest, REST APIs}
\skillLine{ML \& AI}{PyTorch, Transformers, model quantization (llama.cpp 4-bit), edge / on-device inference, JAX, NumPy}
\skillLine{Systems / Performance}{Spark / PySpark, Spark UI profiling, latency \& memory analysis, cache-aware design, Linux perf}
