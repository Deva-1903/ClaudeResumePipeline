...
\begin{itemize}
  \item Founding engineer at an early-stage startup, building backend product from the ground up: a provider-facing \textbf{SMART on FHIR} clinical decision support app (Next.js + Firebase/GCP) that surfaces covered, guideline-backed medication alternatives inside the EHR prescribing workflow to prevent prior authorization.
  \item Extending a \textbf{Firebase Cloud Functions} backend that automates medication-alternative generation from \textbf{RxNorm} normalization, formulary/prior-auth logic, and PubMed/guideline evidence, with \textbf{Gemini} constrained to synthesis while the backend deterministically controls eligibility and coverage.
\end{itemize}

\expItem{Culture \& Morality Lab, UMass Amherst}{Lab Assistant}{Amherst, MA}{Jul 2026 -- Present}
\begin{itemize}
  \item Sole engineer building the lab's CCR text-analysis platform (Python, \textbf{FastAPI}, sentence-transformers, React), owning architecture and all technical decisions end to end while productionizing a published NLP method into a self-serve web tool for researchers.
...
\begin{itemize}
  \item Built an edge-native AI research agent end to end in \textbf{TypeScript} that fetches web sources and streams multi-step digests over WebSocket, with durable workflows (WorkflowEntrypoint + Durable Objects) that survive process restarts and tool failures and persist state in D1 with no external database.
\end{itemize}

\projItem{align-ops -- LLM Post-Training + Serving Ops}{PyTorch, TRL, vLLM, FastAPI, Docker}
\begin{itemize}
  \item Built a backend serving stack, a \textbf{FastAPI} gateway over a vLLM server with Redis caching and async request micro-batching, lifting throughput \textbf{1.9x} and GPU utilization \textbf{52\%$\to$84\%} at \textbf{640 ms} p95 time-to-first-token; containerized with Docker and instrumented with Prometheus/Grafana.
\end{itemize}
...
\skillLine{Backend \& APIs}{FastAPI, Node.js, Express, React, REST APIs, async services / queues, authentication \& rate limiting}
\skillLine{ML \& AI}{PyTorch, Hugging Face Transformers, sentence-transformers, LLM agents, RAG, evaluation}
\skillLine{Systems \& Testing}{Spark / PySpark, Linux profiling, distributed systems, pytest, integration testing, CI/CD}
\skillLine{Cloud \& DevOps}{AWS, GCP, Azure, Docker, Kubernetes, Git, GitHub Actions}
