...
\begin{itemize}
  \item Founding engineer building full-stack features (Next.js/React front end, Firebase/GCP backend) for a provider-facing \textbf{SMART on FHIR} clinical decision support app, in a fast-paced early-stage environment.
...
\begin{itemize}
  \item Sole engineer building the lab's text-analysis platform (\textbf{Python}, \textbf{FastAPI}, sentence-transformers, React, \textbf{Docker}), productionizing a published NLP method into a self-serve web tool with full ownership of architecture and technical decisions delegated by the PI.
  \item Built a YAML model registry, a versioned construct library (\textbf{99 constructs from 38 questionnaires}), and structured data-quality warnings; grew the backend suite to \textbf{40 tests} running hermetically in CI without ML dependencies via a deterministic fake embedding backend.
\end{itemize}
...
\begin{itemize}
  ...
  \item Architected a full-stack platform that replaced manual spreadsheet workflows for the AI team, cutting manual work by \textbf{100+ hours/month} and lifting labeling throughput by \textbf{60\%} (React, Node.js, MongoDB).
  \item Hardened backend reliability across the eTriage stack, adding \textbf{integration tests} and regression protection, hardening authentication and rate limiting, and remediating \textbf{20+ VAPT findings} against sensitive clinical data.
...
\begin{itemize}
  \item Built a multi-stage agentic web-discovery service (Python, FastAPI) with typed retrieval planning, deterministic extraction with LLM fallback, and cell-level provenance; hardened with \textbf{150+ automated tests} and deployed as a live service on \textbf{Google Cloud Run}.
\end{itemize}
...
\begin{itemize}
  \item Cut end-to-end Spark ETL runtime \textbf{$\sim$25\%} (17--19 min to 14.3 min) on a \textbf{$\sim$1.4B-row} NYC TLC dataset via broadcast joins, Adaptive Query Execution, and shuffle-partition tuning; profiled stage-level timings in the Spark UI to localize bottlenecks.
\end{itemize}
...
\begin{itemize}
  \item Built a Python agent that proposes queries and labels, scores them against multiple retrieval systems, and commits only accepted updates back to \textbf{Git} with report cards, guarded by schema checks, duplicate rejection, and read-only holdout protection for a reproducible, auditable workflow.
\end{itemize}
...
%----------SKILLS----------
\section{Skills}
\skillLine{Languages}{Python, Bash, SQL, C++, TypeScript, JavaScript, Java}
\skillLine{Testing \& CI}{pytest, integration testing, automated testing, CI/CD, GitHub Actions, Git}
\skillLine{Systems \& Linux}{Linux, Spark / PySpark, perf profiling, distributed systems, reproducible benchmarking}
\skillLine{Backend \& APIs}{Node.js, Express, FastAPI, React, REST APIs, MongoDB, PostgreSQL}
\skillLine{Cloud \& DevOps}{AWS, GCP, Azure, Docker, Kubernetes}
