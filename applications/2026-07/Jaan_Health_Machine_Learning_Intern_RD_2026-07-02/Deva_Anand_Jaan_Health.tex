...
\begin{itemize}
  ...
  \item Handled sensitive healthcare data in a regulated domain with \textbf{AWS KMS} encryption for PII/clinical data, 90-day retention, auth hardening, rate limiting, and \textbf{20+ VAPT} remediations.
...
\begin{itemize}
  \item Integrated LLM-powered context-aware replies into a conversational chat product and shipped a production LLM feature (\textbf{4-bit quantized Llama-7B via llama.cpp}) with prompt iteration, response-quality heuristics, and content-safety filtering.
...
\begin{itemize}
  \item Built a multi-stage agentic RAG service that turns open-ended queries into structured entity tables via retrieval planning, reranking, and deterministic extraction with LLM fallback and \textbf{cell-level provenance}; hardened with \textbf{150+ tests}.
\end{itemize}
...
\begin{itemize}
  \item Built the multi-view retrieval component of a KG-guided RAG pipeline (query-side decomposition, RRF fusion, cross-encoder reranking, MMR), lifting supporting-fact recall \textbf{+2.68\%} over the KG\textsuperscript{2}RAG baseline on 4{,}905 questions.
\end{itemize}
...
\begin{itemize}
  \item Built an evaluation-improvement agent that proposes queries and labels, scores them against multiple retrieval systems, and accepts only discriminative-power-raising updates, with anti-Goodharting guardrails (schema checks, read-only holdout).
\end{itemize}
...
\skillLine{Frameworks / Tools}{FastAPI, Node.js, React, Docker, Git, GitHub Actions, pytest, REST APIs}
\skillLine{ML \& AI}{PyTorch, TensorFlow, Hugging Face Transformers, scikit-learn, JAX, NumPy, deep learning, NLP, transformers}
\skillLine{LLM \& Retrieval}{LLM APIs (OpenAI/Groq/Ollama), RAG, prompt engineering, LLM evaluation, cross-encoder reranking, RRF, MMR}
