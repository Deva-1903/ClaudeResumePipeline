...
\begin{itemize}
  \item Designed backend APIs for a multi-tenant NHS eTriage system (Node.js, MongoDB) serving \textbf{8+ UK clinical clients} and \textbf{10{,}000+ monthly submissions} via Mayden iaptus REST integrations; contributed to \textbf{\$340K+} in revenue.
  \item Architected a full-stack data-annotation and labeling platform (React, Node.js, MongoDB) used daily by the AI team to structure ML training data, cutting manual annotation by \textbf{100+ hours/month} and lifting throughput by \textbf{60\%}.
  \item Owned backend reliability and performance: added integration tests, monitored production logs and debugged incidents, hardened auth and rate limiting with \textbf{AWS KMS} encryption, and remediated \textbf{20+ VAPT findings}.
...
\begin{itemize}
  \item Built and deployed \textbf{Underdogs Fitness} (\href{https://www.underdogsfitness.in/}{Website}), a production MERN platform with git-webhook auto-deploy, cron automation, and Stripe payments; live \textbf{3+ years}.
\end{itemize}
...
\begin{itemize}
  \item Optimized C++ matrix multiplication through cache-aware tiling, x86 \textbf{AVX/SIMD} vectorization, register-aware micro-kernels, and \textbf{OpenMP} multithreading, profiling L1/L3/TLB performance with \textbf{perf} and inspecting generated assembly.
\end{itemize}
...
\begin{itemize}
  \item Built a Python/FastAPI backend service with typed retrieval planning, reranking, and deterministic extraction returning \textbf{cell-level provenance}; hardened with \textbf{150+ tests} and provider routing across backends.
\end{itemize}
...
\skillLine{Frameworks / Tools}{FastAPI, Node.js, Express, React, Docker, Git, GitHub Actions, REST APIs}
\skillLine{Systems / Performance}{AVX/SIMD (x86), OpenMP multithreading, cache-aware design, Spark / PySpark, distributed data processing, perf profiling, Linux}
\skillLine{ML \& AI}{PyTorch, Hugging Face Transformers, embedding search, RAG, LLM integration, NumPy, evaluation}
