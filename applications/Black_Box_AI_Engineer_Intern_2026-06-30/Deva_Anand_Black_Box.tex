...
\begin{itemize}
  \item Engineered a multi-tenant NHS eTriage backend (Node.js, MongoDB) for \textbf{8+ UK clinical clients}, processing \textbf{10{,}000+ monthly triage submissions} through REST integrations with Mayden's iaptus enterprise platform; contributed to \textbf{\$340K+} in revenue.
  \item Architected a full-stack ML training-data annotation platform (React, Node.js, MongoDB) used daily by the AI team, replacing spreadsheet-based labeling with structured role-based data workflows that cut manual annotation by \textbf{100+ hours/month} and lifted labeling throughput by \textbf{60\%}.
  ...
  \item Served as primary technical POC for the eTriage backend, ran knowledge transfer onboarding a second developer to on-call readiness in \textbf{$\sim$6 weeks}, and piloted a COO-sponsored GitHub Copilot experiment for AI-assisted development.
\end{itemize}
...
\begin{itemize}
  ...
  \item Migrated core repositories from JavaScript to TypeScript and shipped a production LLM auto-commenting feature (\textbf{4-bit quantized Llama-7B via llama.cpp}) with per-persona prompts, async queued generation, and content-safety filtering.
\end{itemize}
...
\begin{itemize}
  \item Built a multi-stage agentic web-discovery service turning open-ended queries into structured entity tables via typed retrieval planning, reranking, and deterministic extraction with LLM fallback and \textbf{cell-level provenance}; hardened with \textbf{150+ tests} and provider routing across LLM backends.
\end{itemize}
...
\begin{itemize}
  \item Built an edge-native AI research assistant where an agent fetches web sources and streams multi-step research digests over WebSocket, with durable workflow orchestration and D1-backed state surviving process restarts and tool failures.
\end{itemize}
...
\skillLine{Frameworks / Tools}{FastAPI, Node.js, Express, React, REST APIs, Docker, Git, GitHub Actions, Cloudflare Workers}
\skillLine{ML \& AI}{LLM agents, RAG, prompt engineering, PyTorch, Hugging Face Transformers, sentence-transformers, evaluation}
