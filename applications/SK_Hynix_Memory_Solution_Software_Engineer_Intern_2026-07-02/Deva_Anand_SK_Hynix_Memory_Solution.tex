...
\begin{itemize}
  \item Built and deployed \textbf{Underdogs Fitness} (\href{https://www.underdogsfitness.in/}{Website}), a production MERN platform with \textbf{git-webhook auto-deploy} (GitHub webhook to a droplet on push), cron automation, and Stripe payments; live \textbf{3+ years}.
\end{itemize}
...
\begin{itemize}
  \item Built a Python/FastAPI service exposing user-facing APIs for a multi-stage discovery pipeline with provider routing and pipeline instrumentation; hardened with \textbf{150+ automated tests}.
\end{itemize}
...
\begin{itemize}
  \item Optimized C++ matrix multiplication from a naive baseline through cache-aware tiling, \textbf{AVX/SIMD} vectorization, and \textbf{OpenMP}, measuring and validating L1/L3/TLB performance with \textbf{perf} and generated assembly.
\end{itemize}
...
\begin{itemize}
  \item Built an automated evaluation-tooling agent that proposes and validates test cases, scores against multiple systems, and commits only improvements to \textbf{Git} with report cards and anti-Goodharting guardrails.
\end{itemize}
